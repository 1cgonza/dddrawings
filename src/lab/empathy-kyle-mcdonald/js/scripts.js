...
(function() {
  ...
  Cell.prototype.sense = function() {
    if (pmouseX !== 0 || pmouseY !== 0) {
      this.s += sp *
                det(this.x, this.y, pmouseX, pmouseY, mouseX, mouseY) /
                (dist(this.x, this.y, mouseX, mouseY) + 1);
    }
    this.s *= sl;
    this.c += this.s;
    var d = bd * this.s + 0.001;
    stage.ctx.beginPath();
  ...
  function init() {
    for (var i = 0; i < n; i++) {
      var a = i + DDD.random(0, Math.PI / 9, true);
      var r = ((i / n) * centerY * (((n - i) / n) * 3.3)) + DDD.random(-3, 3, true) + 3;
